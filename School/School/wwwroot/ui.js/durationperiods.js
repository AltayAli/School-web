...
$(function () {
    $("#title").html("List of duration periods")
    $("#body").dxDataGrid({
        dataSource: new DevExpress.data.CustomStore({
            key: "code",
            load: function () {
                var url = `${Model.DurationPeriodsUrl}/getdevextremeList`;
                return Model.SendRequest(`/api/get?url=${url}&check=1&origin=1`, "GET", false);
            },
            remove: function (key) {
                var url = `${Model.DurationPeriodsUrl}/delete/${key}`;
    ...
    function selection_changed(data) {
        if (data) {
            $("#edit-link").attr("href", `/forms/form/${Model.DurationPeriodsUrl}?isCreate=0&id=${data.code}`);
        }
    }
    $("#detail-link").hide();
    $("#create-link").attr("href", `/forms/form/${Model.DurationPeriodsUrl}?isCreate=1`);
});
...
let FormCreater = {
    CreateForm: function (model, isCreate) {
        ...
        $("#form").on("submit", function (e) {
            let url = `${Model.DurationPeriodsUrl}/`;
            ...
            Model.SendRequest(`/api/${method}?url=${url}&check=1&origin=1`,"POST", true, JSON.stringify(form.option("formData")))
                .then(() => {
                    window.location.href = `/forms/index/${Model.DurationPeriodsUrl}`
                }); 
            e.preventDefault();
        })
    }
