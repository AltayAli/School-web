...
$(function () {
    $("#body").dxDataGrid({
        dataSource: new DevExpress.data.CustomStore({
            key: "code",
            load: function () {
                var url = `${Model.CountryTypesUrl}/getdevextremelist?culture=az`;
                return Model.SendRequest(`/api/get?url=${url}&check=1&origin=1`, "GET", false);
            },
            remove: function (key) {
                var url = `${Model.CountryTypesUrl}/delete/${key}`;
    ...
    function selection_changed(data) {
        if (data) {
            $("#edit-link").attr("href", `/forms/form/${Model.CountryTypesUrl}?isCreate=0&id=${data.code}`);
        }
    }
    $("#title").html("List of country types");
    $("#detail-link").hide();
    $("#create-link").attr("href", `/forms/form/${Model.CountryTypesUrl}?isCreate=1`);
});
       ...
       var form = $("#form").dxForm({
        ...
        $("#form").on("submit", function (e) {
            ...
            Model.SendRequest(`/api/${method}?url=${url}&check=1&origin=1`,"POST", true, JSON.stringify(form.option("formData")))
                .then(() => {
                    window.location.href = `/forms/index/${Model.CountryTypesUrl}`
                });
