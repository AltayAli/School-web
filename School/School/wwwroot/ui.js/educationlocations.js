...
$(function () {
    $("#title").html("List of education locations")
    $("#detail-link").hide();
    $("#body").dxDataGrid({
        dataSource: new DevExpress.data.CustomStore({
            key: "code",
            load: function () {
                var url = `${Model.EducationLocationsUrl}/getdevextremeList`;
                return Model.SendRequest(`/api/get?url=${url}&check=1&origin=1`, "GET", false);
            },
            remove: function (key) {
                var url = `${Model.EducationLocationsUrl}/delete/${key}`;
    ...
    function selection_changed(data) {
        if (data) {
            $("#edit-link").attr("href", `/forms/form/${Model.EducationLocationsUrl}?isCreate=0&id=${data.code}`);
        }
    }
    $("#create-link").attr("href", `/forms/form/${Model.EducationLocationsUrl}?isCreate=1`);
});
    ...
    CreateForm: function (model, isCreate) {
       ...
       var form =$("#form").dxForm({
        ...
        $("#form").on("submit", function (e) {
            let url = `${Model.EducationLocationsUrl}/`;
            ...
            Model.SendRequest(`/api/${method}?url=${url}&check=1&origin=1`,"POST", true, JSON.stringify(form.option("formData")))
                .then(() => {
                    window.location.href = `/forms/index/${Model.EducationLocationsUrl}`
                });
            e.preventDefault();
        })
    }
